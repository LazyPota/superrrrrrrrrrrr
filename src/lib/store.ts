...
export async function loginUser(email: string, password: string) {
  ...
  const supaResult = await loginWithSupabase(cleanEmail, password);
  if (supaResult.ok && supaResult.user) {
    if (typeof window !== 'undefined') {
      localStorage.setItem(STORAGE_KEYS.USER, JSON.stringify(supaResult.user));
      migrateGuestCart(supaResult.user.email);
    }
    return { ok: true, user: supaResult.user };
  }
