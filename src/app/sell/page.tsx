...
export default function SellPage() {
  ...
  async function onFinish(values: any) {
    ...
    if (!currentUser || !currentUser.email) {
      setFormError('🔒 Sesi telah berakhir. Silakan login kembali.');
      return;
    }
    ...
    const uploadedImages = await Promise.all(
      rawImages.map(img => uploadProductImage(img))
    );

    const coverImage = uploadedImages[0] || (values.image ? await uploadProductImage(values.image.trim()) : '');
